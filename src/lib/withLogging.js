/*
 * BLERP Inc.C ("BLERP") CONFIDENTIAL
...
const createComponentWithData = Component =>
    class WithLogging extends React.Component {
        logAction = logMutation => ({ data, action, event }) => {
            const w =
                window.innerWidth ||
            ...
            // id for tracking campaigns
            const hashTrack = urlParams.get("hashTrack");
            ...
            return (
                <Mutation mutation={LOG_ACTION}>
                    {LOG_ACTION_MUTATION => (
                        <Component
                            {...this.props}
                            logAction={this.logAction(LOG_ACTION_MUTATION)}
                        />
                    )}
                </Mutation>
            );
