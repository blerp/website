...
class CreateBoard extends React.Component {
    ...
    handleSubmit = async () => {
        ...
        try {
            const result = await this.props.createCategoryForWeb({
                variables: {
                    record: {
            ...
            if (
                result.data.errors ||
                !result.data.web.categoryCreate.recordId
            ) {
                // TODO: Make Proper UI For This
    ...
    handleImageUrlChange = event => {
        ...
        this.setState({ imageUrl: value, errorDescription: false });
    };
...
const createBoard = gql`
    mutation makeCategoryForWeb($record: CreateOneCategoryInput!) {
        web {
...
export default compose(graphql(createBoard, { name: "createCategoryForWeb" }))(
    CreateBoard,
);
